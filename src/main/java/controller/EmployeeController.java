...
public class EmployeeController {
    ...
    public static void findEmployeeByName(){
        ...
        TypedQuery<Employee> query = session.createNamedQuery(
                "findEmployeeByName", Employee.class);
        query.setParameter("name", "Tom Thele");
        List<Employee> employees = query.getResultList();
        System.out.printf("%-10s%-22s%-14s%-14s%-16s%-10s%-10s%s%n", "ID",
                "Name", "Salary", "Job", "AddressLine", "Zipcode", "city",
                "startDate");
